...
def Color_Detect(imageFrame):
    ...
    red_lower = np.array([0, 60, 0], np.uint8) #紅色積木判斷範圍的最小值
    red_upper = np.array([5, 255, 255], np.uint8)#紅色積木判斷範圍的最大值
    red_lower2 = np.array([160, 50, 0], np.uint8)#紅色積木判斷範圍的最小值
    ...
    green_upper = np.array([75, 255, 230], np.uint8) #綠色積木判斷範圍的最大值
    ...
    for pic, contour in enumerate(contours):
        ...
        if(area > 100) and area > red_area: #紅色積木面積大於150的時候，表示看到紅色積木了
            red_num += 1
            red_area = area
    ...
    for pic, contour in enumerate(contours):
        green_num += 1
        area = cv2.contourArea(contour)
        if(area > 100) and area > green_area: #綠色積木面積大於150的時候，表示看到綠色積木了
            green_area = area
            x, y, w, h = cv2.boundingRect(contour)
            green_x = x - int(w/3)
            green_y = y+h
...
def Draw_Line (img, red_x, red_y, green_x, green_y, gyro):
    ...
    cv2.line(img, (240, 360), (int(mapping(gyro, 0, 20, 240, 300)), 180), (0, 255, 0), 4)
...
def car_control(img, red_x, red_y, red_area, green_x, green_y,
    ...
    if mode == False:
        if green_y > red_y and green_y < 320 and green_x > mapping(gyro, 20, 100, 0, 240) and green_x < mapping(gyro, -100, -10, 240, 480): #偵測到離機器最近的綠色積木的時候
            if green_y > constrain(mapping(green_x, 0, 240, 203, 220), 200, 220) and green_y < 250 and (abs(gyro) < 15 or logic == 1): #機器距離積木近且陀螺儀是正的時候
                logic = 1
                ...
                Motor.Servo(constrain(angle, -38, 38))
                logic = 0
        elif red_y > green_y and red_y < 350 and red_x > mapping(gyro, 20, 100, 0, 240) and red_x < mapping(gyro, -100, -20, 240, 480): #偵測到離機器最近的紅色積木的時候
            if red_y > 205 and red_y < 230 and (abs(gyro) < 25 or logic == 1): #機器距離積木近且陀螺儀是正的時候
                logic = 1
                R_error = ((red_x - R_wallX) * kp) + (R_error - R_last_error) * kd
                Motor.Servo(constrain(R_error, -38, 38))
            elif red_y >= 230: #陀螺儀雖然沒有正，但是快撞到積木的時候
                logic = 1
    ...
    if Start: #當matrix mini 按下按鈕後(開始運行後)
        if first_round == False:
            ...
            Motor.DC(constrain(mapping(abs(angle), 0, 20, 55, 45), 55, 45))
    elif mode == False:
        Motor.DC(0)
    ...
    try:
        while Run:
            ...
            while Line_num < 24 and Run:   #圈數設定
                ...
                if Line_num%2 == 0 and error_time - last_time > 2.5:
                    Line_num = Line_num+1
                    print("[Color_Sensor]: Line_Count: {}".format(Line_num))
...
while Run:
    ...
    roi_image[0:183, 0:480] = [0,0,0]#遮罩鏡頭畫面的範圍
    ...
    if Key == ord('q'): #按下Q鍵停止程式
        ...
        End = False
        break
    if Key == ord('w') or button: #按下W鍵或按下matrix mini的按鈕汽車開始運行
...
s = time.time()
while time.time() - s < 2.4:#Run == False:
    roi_image = image.copy()
    roi_image[0:185, 0:480] = [0,0,0]#遮罩鏡頭畫面的範圍
